...
const ProductDetails = ({ productData, similarProducts }) => {
    ...
    useEffect(() => {
        const originalMainImage = productData?.image;
    ...
    }, [isFullscreenOpen, mainImage, secondImages]);
    ...
    const showPreviousImage = () => {
        if (secondImages.length < 2) return;

        const currentIndex = secondImages.indexOf(mainImage);
        const previousIndex =
            currentIndex <= 0
                ? secondImages.length - 1
                : currentIndex - 1;

        setMainImage(secondImages[previousIndex]);
    };
    ...
    const showNextImage = () => {
        ...
        const nextIndex =
            currentIndex === -1 ||
            currentIndex >= secondImages.length - 1
                ? 0
                : currentIndex + 1;

        setMainImage(secondImages[nextIndex]);
    };
                        ...
                        >
                            {secondImages.length > 1 && (
                                <button
                                    ...
                                    className="absolute left-2 z-10 flex h-12 w-12 items-center justify-center border border-white/20 bg-black/55 text-3xl text-white transition hover:border-white hover:bg-white hover:text-black sm:left-6 sm:h-14 sm:w-14"
                            ...
                            {secondImages.length > 1 && (
                                <button
                                    ...
                                    className="absolute right-2 z-10 flex h-12 w-12 items-center justify-center border border-white/20 bg-black/55 text-3xl text-white transition hover:border-white hover:bg-white hover:text-black sm:right-6 sm:h-14 sm:w-14"
                            ...
                            {secondImages.length > 1 && (
                                <span className="absolute bottom-3 left-1/2 -translate-x-1/2 border border-white/15 bg-black/55 px-4 py-2 text-[10px] font-bold tracking-[0.16em] text-white/65 backdrop-blur-md sm:bottom-6">
                                    {Math.max(
                                        secondImages.indexOf(mainImage) + 1,
                                        1
                                    )}{" "}
                                    / {secondImages.length}
                                </span>
