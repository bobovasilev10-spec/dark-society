...
const Category = ({ categoryData }) => {
    ...
    return (
        <div className="">
            ...
            {showSubCategory &&
                <ul className="leading-8 border rounded-b-xl px-2">
                    {categoryData.children.length > 0 && categoryData.children.map((item, index) => (
                        <a key={index} href={`/category/${categoryData.slug}/${item.slug}`}>
                            <li  className="py-1 hover:underline underline-offset-4 cursor-pointer text-xl border-b-2 border-gray-700">{item.name[language]}</li>
                        </a>
                    ))}
                </ul>
            }
